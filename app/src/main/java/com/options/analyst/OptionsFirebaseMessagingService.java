 ...
 * 2. onMessageReceived() — когда приходит push
 *
...
public class OptionsFirebaseMessagingService extends FirebaseMessagingService {
    ...
    //
    // АРХИТЕКТУРА data-only (фикс фона на MIUI):
    // Worker шлёт ТОЛЬКО data-блок, без notification. Это критично:
    //  • notification-блок отдаёт показ баннера системе, а MIUI его давит в фоне
    //    для sideload-приложений → onMessageReceived НЕ вызывается, баннера нет.
    //  • data-only с priority:high ОБЯЗЫВАЕТ FCM-SDK разбудить onMessageReceived()
    //    даже в фоне/Doze. Баннер рисует НАШ код через nm.notify() на нашем канале
    //    price_alerts — MIUI его не давит, потому что это локальное уведомление.
    //
    // Поэтому баннер рисуем ВСЕГДА (и в фоне, и на переднем плане). Прежней ветки
    // "в фоне выходим по return" больше нет — именно она убивала показ.
    // ─────────────────────────────────────────────────────────────────
    @Override
    public void onMessageReceived(RemoteMessage remoteMessage) {
        ...
        // data-only архитектура: notification-блока больше нет, баннер рисуем ВСЕГДА сами.
        // Раньше тут стоял `if (!isForeground && hasNotif) return;` — он и убивал показ
        // в фоне, т.к. система (MIUI) notification-блок давила, а наш код выходил по return.
        // Теперь рисуем всегда — и в фоне, и на переднем плане.
        ...
        // Fallback: поле notification (на случай если прилетит notification-блок)
    ...
    // ─────────────────────────────────────────────────────────────────
    // Определяет, на переднем ли плане приложение. Оставлено для debug-маяка
    // (видеть в каком состоянии пришёл push). На показ баннера больше не влияет —
    // в data-only архитектуре рисуем всегда.
    // ─────────────────────────────────────────────────────────────────
